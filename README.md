...
函数可以重载这一特性是C++相对于C语言最明显的改进点之一。如果在同一个作用域中定义了多个名称相同，但是参数列表不同的函数，在C++中便形成了函数重载。需要注意的是，main函数不允许被重载。

### 1. 定义重载函数

如果我们需要处理一些相似的功能时，需要定义一些名称相同，但是参数数量或类型不同的函数，这些函数便形成了重载函数。在编译时，编译器根据实参的类型选择应该调用哪一个函数。
